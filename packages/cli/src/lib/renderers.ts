...
// ─── Generic renderer for Cursor/Copilot/Windsurf ───
...
function renderGenericRules(ir: ContextIR, buildHeader: (ctx: RenderHeader) => void): string {
  const lines: string[] = [];
  const now = new Date().toISOString().slice(0, 10);

  buildHeader({ lines, ir, now });
...
export function renderCursor(ir: ContextIR): string {
  return renderGenericRules(ir, ({ lines, ir: ctx, now }) => {
    lines.push(`# Cursor Rules — ${ctx.project.name}`);
...
export function renderWindsurf(ir: ContextIR): string {
  return renderGenericRules(ir, ({ lines, ir: ctx, now }) => {
    lines.push(`# Windsurf Rules — ${ctx.project.name}`);
    lines.push(`# Generated by Orbit CLI on ${now}`);
    lines.push('');
  });
}
